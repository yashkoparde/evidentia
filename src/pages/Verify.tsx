...
<<<<<<< HEAD
                    <div className="text-center space-y-4 z-10 w-full px-4 flex flex-col items-center">
                      <div className="relative flex items-center justify-center p-3 bg-white rounded-lg mb-2">
                        <QRCodeSVG 
                          value={secureLinkInput || "https://evidentia.protocols"}
                          size={100}
                          level="M"
                          includeMargin={false}
                        />
                        <motion.div 
                          ...
                          className="absolute -inset-4 rounded-xl border border-dashed border-evidentia-accent/50"
                        />
                      </div>
                      
                      <div className="space-y-1">
                        <div className="text-[10px] font-mono text-evidentia-accent font-bold tracking-widest uppercase animate-pulse">
...
<<<<<<< HEAD
                    ...
                    <motion.div 
                      ...
                      className="w-full text-center space-y-4 z-10 flex flex-col items-center p-2"
                    >
                      <div className="flex items-center gap-2 px-3 py-1 bg-evidentia-success/10 border border-evidentia-success/20 rounded-full text-[8.5px] font-mono text-evidentia-success uppercase tracking-widest font-bold">
                        <ShieldCheck className="w-3.5 h-3.5 text-evidentia-success animate-pulse" /> DECRYPTED SHARE DOSSIER
                      </div>

                      <div className="w-14 h-14 bg-black/40 rounded-xl border border-white/10 flex items-center justify-center overflow-hidden shadow-inner relative mt-1">
                        {resolvedEvidence.thumbnail ? (
                          <img src={resolvedEvidence.thumbnail} alt="" className="w-full h-full object-cover" />
                        ) : (
                          React.createElement(getFileIcon(resolvedEvidence.fileType, resolvedEvidence.fileName), { className: "w-7 h-7 text-evidentia-accent" })
                        )}
                      </div>

                      <div className="space-y-1 w-full px-2 text-center">
                        <h4 className="text-xs font-display font-black text-white truncate uppercase tracking-tight">{resolvedEvidence.title}</h4>
                        <p className="text-[9px] font-mono text-evidentia-accent font-bold uppercase tracking-widest">{resolvedEvidence.caseId}</p>
                        <div className="text-[8px] font-mono text-white/40 uppercase truncate bg-white/5 py-1 px-2 rounded inline-block max-w-full font-mono">
                          {resolvedEvidence.fileHash.substring(0, 32)}...
                        </div>
                      </div>

                      <div className="grid grid-cols-2 gap-2 w-full pt-1">
                        <div className="p-2 bg-white/[0.02] border border-white/5 rounded text-left">
                          <p className="text-[7px] font-mono uppercase tracking-widest text-white/30">LOCKED ON LEDGER</p>
                          <p className="font-mono text-[9px] text-emerald-400 font-bold uppercase mt-0.5 truncate">POLYGON MAINNET</p>
                        </div>
                        <div className="p-2 bg-white/[0.02] border border-white/5 rounded text-left">
                          <p className="text-[7px] font-mono uppercase tracking-widest text-white/30">RISK INDEX</p>
                          <p className="font-mono text-[9px] text-white/80 font-bold uppercase mt-0.5">
                            {resolvedEvidence.aiRiskScore !== undefined ? `${resolvedEvidence.aiRiskScore}%` : '0%'}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col gap-1.5 w-full pt-1">
                        <button
                          onClick={() => onEvidenceClick(resolvedEvidence.id)}
                          className="w-full py-2 bg-evidentia-accent text-black font-mono text-[10px] font-bold rounded-lg hover:bg-white hover:shadow-[0_0_15px_#00F0FF] transition-all cursor-pointer uppercase tracking-widest"
                        >
                          OPEN SECURE CASE DOSSIER
                        </button>
                        
                        <button
                          onClick={() => {
                            setLinkScanStatus('idle');
                            setSecureLinkInput('');
                            setResolvedEvidenceId(null);
                            setLinkScanLogs([]);
                          }}
                          className="w-full py-2 bg-white/5 border border-white/10 hover:bg-white/10 font-mono text-[9px] rounded-lg text-white/50 hover:text-white transition-all cursor-pointer uppercase tracking-widest"
                        >
                          RESET SCANNER
                        </button>
                      </div>
...
<<<<<<< HEAD
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-[9px] font-mono text-evidentia-danger/80 uppercase tracking-widest px-4">Secure signature invalid or unrecognized.</p>
                        <button
                          onClick={() => {
                            setLinkScanStatus('idle');
                            setSecureLinkInput('');
                          ...
                          className="mt-2 px-3 py-1 bg-white/5 border border-white/10 font-mono text-[8px] rounded uppercase hover:bg-white/10 text-white"
                        >
                          Clear
                        </button>
                      </div>
                    ...
                    <div className="text-center space-y-4 z-10 flex flex-col items-center">
                      <div className="relative p-2.5 bg-white rounded-lg animate-in fade-in zoom-in-95 duration-300">
                        <QRCodeSVG 
                      ...
                      <div className="space-y-1 px-4">
                        <p className="text-[10px] font-mono text-evidentia-accent uppercase tracking-widest animate-pulse font-bold">🎯 TARGET DISCOVERED</p>
                        <p className="text-[8px] font-mono text-white/30 max-w-[180px] uppercase">
                          Initialize scan on the right panel to test secure signature.
                        </p>
                      </div>
                    </div>
                  ) : (
